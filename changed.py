...
def game_scene():
    ...
    mute = False
    sprite_speed = constants.SPRITE_MOVEMENT_SPEED
...
def show_alien():
    ...
    a_bonus_alien = stage.Sprite(image_bank_bonus_sprites,9,
                               constants.OFF_SCREEN_X,
                               constants.OFF_SCREEN_Y)
    for alien_number in range(constants.TOTAL_NUMBER_OF_ALIENS):
        if random.randint(1,10) <= 5:
            aliens.append(a_bonus_alien)
        else:
    ...
    while True:
        ...
        if keys & ugame.K_X != 0:
            ...
            if keys & ugame.K_DOWN != 0:
                #speed up button
                if sprite_speed == constants.SPRITE_MOVEMENT_SPEED:
                    sprite_speed = 2 * constants.SPRITE_MOVEMENT_SPEED
                else:
                    sprite_speed = constants.SPRITE_MOVEMENT_SPEED
        ...
        for alien_number in range(len(aliens)):
            if aliens[alien_number].x > 0:
                ...
                if aliens[alien_number].y > constants.SCREEN_Y:
                    aliens[alien_number].move(aliens[alien_number].x, 0)
                ...
                for alien_number in range(len(aliens)):
                    if aliens[alien_number].x > 0:
                        if stage.collide(lasers[laser_number].x + 6, lasers[laser_number].y + 2,
                            ...
                            show_alien()
                            show_alien()
                            if aliens[alien_number] == a_bonus_alien:
                                score = score + 2
                            else:
                                score = score + 1
